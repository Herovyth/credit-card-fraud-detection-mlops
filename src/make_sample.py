...
def main():
    ...
    df = pd.read_csv(src)
    print(f"Повний датасет: {df.shape} | шахрайство: {df['Class'].sum()} ({df['Class'].mean()*100:.3f}%)")
    ...
    print(f"Розмір: {sample.shape} | шахрайство: {sample['Class'].sum()} ({sample['Class'].mean()*100:.2f}%)")
    print(f"\nДодай файл у Git:")
