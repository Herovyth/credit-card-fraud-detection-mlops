...
def build_model(model_type: str, params: Dict[str, Any], seed: int) -> Any:
    ...
    raise ValueError(
        f"Невідомий model.type='{model_type}'. Очікується: random_forest | logistic_regression"
    )
    ...
    if model_type == "random_forest":
        ...
        return {
            "n_estimators": trial.suggest_int(
                "n_estimators", sp.n_estimators.low, sp.n_estimators.high
            ),
            "max_depth": trial.suggest_int(
                "max_depth", sp.max_depth.low, sp.max_depth.high
            ),
            "min_samples_split": trial.suggest_int(
                "min_samples_split", sp.min_samples_split.low, sp.min_samples_split.high
            ),
            "min_samples_leaf": trial.suggest_int(
                "min_samples_leaf", sp.min_samples_leaf.low, sp.min_samples_leaf.high
            ),
        }
    if model_type == "logistic_regression":
...
def make_sampler(
    sampler_name: str, seed: int, grid_space: Dict = None
    ...
    if name == "grid":
        if not grid_space:
            raise ValueError(
                "Для sampler='grid' потрібно задати grid_space у конфігурації."
            )
        return optuna.samplers.GridSampler(search_space=grid_space)
    raise ValueError(
        f"Невідомий sampler '{sampler_name}'. Очікується: tpe | random | grid"
    )
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        with mlflow.start_run(nested=True, run_name=f"trial_{trial.number:03d}"):
            ...
            if cfg.hpo.use_cv:
                ...
                score = evaluate_cv(
                    model, X_all, y_all, cfg.hpo.metric, cfg.seed, cfg.hpo.cv_folds
                )
            else:
                score = evaluate(
...
def main(cfg: DictConfig) -> None:
    ...
    mlflow.set_tag(
        "train_data_md5",
        file_md5(str(project_root / cfg.data.prepared_dir / "train.csv")),
    )
    mlflow.set_tag(
        "test_data_md5",
        file_md5(str(project_root / cfg.data.prepared_dir / "test.csv")),
    )
    ...
    with mlflow.start_run(
        ...
        final_score = evaluate(
            best_model, X_train, y_train, X_test, y_test, cfg.hpo.metric
        )
        ...
        if cfg.mlflow.register_model:
            ...
            register_model_if_enabled(
                model_uri, cfg.mlflow.model_name, cfg.mlflow.stage
            )
