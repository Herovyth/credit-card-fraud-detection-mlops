    ...
    def test_feature_ranges(self, sample_df):
        ...
        for col in v_cols:
            assert np.isfinite(sample_df[col]).all(), (
                f"Колонка {col} містить inf або NaN"
            )
...
class TestArtifactsPostTrain:
    ...
    def test_model_pkl_exists(self):
        assert (PROJECT_ROOT / "models/best_model.pkl").exists(), "best_model.pkl не знайдено"
    ...
    def test_confusion_matrix_exists(self):
        assert (PROJECT_ROOT / "models/confusion_matrix.png").exists(), (
            "confusion_matrix.png не знайдено"
        )
    ...
    def test_metrics_values_in_range(self, metrics):
        for key, val in metrics.items():
            assert 0.0 <= float(val) <= 1.0, (
                f"Метрика '{key}' поза межами [0,1]: {val}"
            )
    ...
    def test_quality_gate_roc_auc(self, metrics):
        ...
        assert roc_auc >= 0.85, (
            f"ROC-AUC занадто низький: {roc_auc:.4f} < 0.85"
        )